<x-admin-layout>
    <div class="max-w-7xl mx-auto space-y-6 animate-fade-in pb-10">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 py-4">
            <div>
                <h2 class="text-3xl font-black tracking-tight text-slate-900">Peserta Bimbingan</h2>
                <p class="text-sm font-medium text-slate-500 mt-1">Semua peserta yang dibimbing oleh <span class="font-bold text-slate-900">{{ $mentor->name }}</span>.</p>
            </div>
        ...
        <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden transition-all duration-500">
            ...
            <div class="px-6 pt-6 pb-4 border-b border-slate-100 bg-slate-50/60">
                <form method="GET" action="{{ route('super.mentors.interns', $mentor->id) }}" class="max-w-md">
                    <div class="relative">
                        ...
                        >
                        @if(request('search'))
                            <a href="{{ route('super.mentors.interns', $mentor->id) }}" class="absolute inset-y-0 right-3 flex items-center text-[10px] font-black uppercase tracking-widest text-slate-300 hover:text-rose-500">
                                Reset
                            </a>
                        @endif
                    </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100 text-slate-700 font-bold text-sm">
                        ...
                        @empty
                        <tr>
                            <td colspan="4" class="px-8 py-20 text-center text-slate-300 font-black uppercase tracking-widest text-xs italic">Belum ada peserta bimbingan.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
