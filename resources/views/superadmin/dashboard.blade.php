<x-admin-layout>
    <div class="space-y-8">
        ...
        <div class="bg-white rounded-3xl border border-slate-200 p-6">
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                <div>
                    <h2 class="text-2xl font-black tracking-tight">Dashboard Admin</h2>
                    <p class="text-sm text-slate-500">Tanggal {{ $today }}</p>
                </div>
                <div class="flex gap-3 items-center">
                    <a href="{{ route('super.mentors.index') }}" class="px-4 py-2 bg-indigo-600 text-white rounded-xl text-xs font-bold uppercase tracking-widest hover:bg-indigo-700">Instruktur</a>
                    <a href="{{ route('super.interns.all') }}" class="px-4 py-2 bg-slate-100 text-slate-700 rounded-xl text-xs font-bold uppercase tracking-widest hover:bg-slate-200">Peserta</a>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <div class="bg-gray-50 rounded-2xl border border-slate-200 p-6">
                <div class="flex items-center justify-between">
                    <p class="text-sm font-medium text-slate-500">Total Instruktur</p>
                    <svg class="w-5 h-5 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0"></path></svg>
                </div>
                <div class="mt-3 flex items-end gap-2">
                    <span class="text-3xl font-extrabold">{{ $mentorCount }}</span>
                    <span class="text-xs opacity-50">akun</span>
                </div>
            </div>
            <div class="bg-gray-50 rounded-2xl border border-slate-200 p-6">
                <div class="flex items-center justify-between">
                    <p class="text-sm font-medium text-slate-500">Total Peserta</p>
                    <svg class="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7a4 4 0 11-8 0 4 4 0 018 0zM15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197"></path></svg>
                </div>
                <div class="mt-3 flex items-end gap-2">
            ...
            <div class="bg-gray-50 rounded-2xl border border-slate-200 p-6">
                <div class="flex items-center justify-between">
                    <p class="text-sm font-medium text-slate-500">Laporan Pending</p>
            ...
            <div class="bg-gray-50 rounded-2xl border border-slate-200 p-6">
                <div class="flex items-center justify-between">
                    <p class="text-sm font-medium text-slate-500">Laporan Diterima</p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2 bg-white rounded-3xl border border-slate-200 overflow-hidden">
                ...
                <div class="px-8 py-6">
                    <div id="chartStatusAdmin" style="height: 320px"></div>
                </div>
            </div>
            <div class="bg-white rounded-[2.5rem] border border-slate-200 overflow-hidden">
                <div class="px-8 py-6 border-b border-slate-100 flex items-center justify-between">
                    <h3 class="text-lg font-bold">Pending Terkini</h3>
                    <a href="{{ route('super.interns.all') }}" class="text-[10px] font-black uppercase tracking-widest text-indigo-600">Lihat</a>
                </div>
                <div class="divide-y divide-slate-100">
                    @forelse($pendingReports->take(5) as $report)
    ...
    <script>
        (function() {
            ...
            var el = document.querySelector('#chartStatusAdmin');
